...
from app.models import QueryRequest, QueryResponse, IngestionResponse, HealthResponse
    ...
    try:
        ...
        if UPLOAD_DIR.exists():
            for pdf_file in UPLOAD_DIR.glob("*.pdf"):
                ...
                documents.append(
                    {"filename": pdf_file.name, "size": file_size, "size_mb": round(file_size / (1024 * 1024), 2)}
                )
...
async def health_check():
    ...
    try:
        ...
        return HealthResponse(status="unhealthy", vector_db_initialized=False, total_documents=0, total_chunks=0)
