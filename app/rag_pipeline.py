...
class RAGPipeline:
    ...
    def __init__(self):
        ...
        self.vector_db = VectorDatabase(dimension=1024, storage_path="vectordb")  # Mistral embed dimension
        self.mistral_client = MistralClient()
        self.chunker = TextChunker(chunk_size=settings.chunk_size, chunk_overlap=settings.chunk_overlap)
        self.hybrid_search = HybridSearch(
            vector_db=self.vector_db, semantic_weight=settings.semantic_weight, keyword_weight=settings.keyword_weight
        )
    ...
    async def ingest_pdf(self, pdf_path: str, filename: str) -> Dict:
        ...
        try:
            ...
            metadata = {"source_file": filename, "ingestion_time": time.time()}
            ...
            for (chunk_id, chunk_text, chunk_meta), embedding in zip(chunks, embeddings):
                entries.append((chunk_id, np.array(embedding, dtype=np.float32), chunk_text, chunk_meta))
    ...
    async def query(
        ...
        try:
            ...
            if intent in [Intent.GREETING, Intent.CHITCHAT]:
                return self._create_simple_response(intent, start_time)
            ...
            if use_hybrid_search:
                logger.debug(
                    f"Performing hybrid search (top_n={self.top_n_semantic} semantic, "
                    f"top_k={self.top_k_keyword} keyword)..."
                )
                ...
                results = self.vector_db.search(query_vector=np.array(query_embedding, dtype=np.float32), top_k=top_k)
            ...
            if not results or results[0][score_key] < self.similarity_threshold:
                return QueryResponse(
                    answer=(
                        ...
                        "The available documents may not contain relevant information."
                    ),
                    citations=[],
                    intent=intent,
            ...
            is_summary_query = any(
                word in query.lower() for word in ["summarize", "summary", "overview", "list", "all", "document"]
            )
            ...
            if enable_hallucination_check and answer and not is_summary_query:
                ...
                verification = await self.mistral_client.detect_hallucination(answer, context_chunks)

                if verification["is_hallucination"]:
                    logger.warning("Hallucination detected in answer")
                    answer = (
                        "I found relevant information, but I cannot provide a fully "
                        "supported answer. Please review the source documents directly."
                    )
            ...
            return QueryResponse(
                answer=answer, citations=citations, intent=intent, confidence=confidence, processing_time_ms=elapsed
            )
            ...
            return QueryResponse(
                answer=(
                    "I encountered an error while processing your question. This might be "
                    "due to complex or ambiguous queries. Please try rephrasing your "
                    "question or being more specific about which document you'd like me to "
                    "reference."
                ),
                citations=[],
    ...
    def _create_simple_response(self, intent: Intent, start_time: float) -> QueryResponse:
        """Create a response for non-retrieval queries (greetings, etc.)"""
        responses = {
            Intent.GREETING: (
                "Hello! I'm here to help answer questions about your documents. " "Feel free to ask me anything!"
            ),
            Intent.CHITCHAT: (
                "I'm a document assistant designed to help you find information "
                "in your uploaded PDFs. What would you like to know?"
            ),
        }
